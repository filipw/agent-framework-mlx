...
class MLXChatClient(BaseChatClient):
    ...
    def _get_sampler(self, options: Optional[MLXChatOptions] = None):
        ...
        if options:
            ...
            # Additional properties are directly in the options dict
            if "min_p" in options:
                config["min_p"] = options["min_p"]
            if "top_k" in options:
                config["top_k"] = options["top_k"]
            if "xtc_probability" in options:
                config["xtc_probability"] = options["xtc_probability"]
            if "xtc_threshold" in options:
                config["xtc_threshold"] = options["xtc_threshold"]
    ...
    def _get_logits_processors(self, options: Optional[MLXChatOptions] = None):
        ...
        if options:
            ...
            if "repetition_context_size" in options:
                config["repetition_context_size"] = options["repetition_context_size"]
